...
func (c *CouponDatatbase) ApplyCoupon(userId int, couponCode string) (int, error) {
	tx := c.DB.Begin()
	var coupon domain.Coupons
	...
	if err != nil {
		tx.Rollback()
		return 0, fmt.Errorf("The coupon was already used")
	}
	...
	log.Println(cartdetails.CouponId)
...
func (c *CouponDatatbase) RemoveCoupon(userId int) error {
	tx := c.DB.Begin()
